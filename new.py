...
if match:
    ...
    # Remove all symbols and the "^" symbol from the version number(s)
    cleaned_version_numbers = [re.sub(r'[^\d\.]', '', version_number).replace("^", "") for version_number in version_numbers]

    if len(cleaned_version_numbers) == 1:
        # Update the Solidity version number in the input file
        updated_contents = re.sub(pattern, 'pragma solidity ' + cleaned_version_numbers[0] + ';', example_string)
